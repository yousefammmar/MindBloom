/**
 * MindBloom — Dashboard & Auth Logic
 * Backed by a real Express + SQLite API (see /backend and api-client.js).
 */
...
// ─────────────────────────────────────────────────────────────
// Auth Manager — talks to the MindBloom API (backend/routes/auth.js)
// ─────────────────────────────────────────────────────────────
const AuthManager = {

    /** Called after the API confirms a user is signed in (fresh login, register, or restored session) */
    async onLogin(user) {
        state.currentUser = { ...user };
        this.applyUserTheme(user.theme || 'forest');
        await loadState();
    ...
    applyUserTheme(themeName) {
        document.body.setAttribute("data-theme", themeName || "forest");
        ...
        if (themeName === "ramadan") {
            ...
            if (nameEl) nameEl.textContent = "Ramadan";
            if (menuLabel) menuLabel.textContent = "Switch to Forest 🌿";
        } else {
            if (iconEl) iconEl.textContent = "🌿";
            if (nameEl) nameEl.textContent = "Forest";
            if (menuLabel) menuLabel.textContent = "Switch to Ramadan ✨";
        }
    },
    ...
    async toggleTheme() {
        ...
        const newTheme = current === 'ramadan' ? 'forest' : 'ramadan';
        state.currentUser.theme = newTheme;
        this.applyUserTheme(newTheme);
        try {
            await api.updateProfile({ theme: newTheme });
        } catch (e) { /* theme still applied locally even if sync fails */ }
        showToast(`Theme switched to ${newTheme === 'ramadan' ? 'Ramadan ✨' : 'Forest 🌿'}`, 'info');
    },
    ...
    updateHeaderProfile() {
        ...
        if (user) {
            ...
            const color = user.avatarColor || "#c084fc";

            if (headerAvatar) {
                if (user.avatarUrl) {
                    headerAvatar.innerHTML = `<img src="${user.avatarUrl}" style="width:100%;height:100%;border-radius:50%;object-fit:cover;" />`;
                } else {
                    headerAvatar.textContent = initial;
                    headerAvatar.style.background = `linear-gradient(135deg, ${color}, #f472b6)`;
                }
            }
            if (headerName) headerName.textContent = user.fullName ? user.fullName.split(' ')[0] : user.email;
            if (menuAvatar) {
                if (user.avatarUrl) {
                    menuAvatar.innerHTML = `<img src="${user.avatarUrl}" style="width:100%;height:100%;border-radius:50%;object-fit:cover;" />`;
                } else {
                    menuAvatar.textContent = initial;
                    menuAvatar.style.background = `linear-gradient(135deg, ${color}, #f472b6)`;
                }
            }
            if (menuFullName) menuFullName.textContent = user.fullName || user.email;
            if (menuEmail) menuEmail.textContent = user.email || "";
        } else {
            if (headerAvatar) { headerAvatar.textContent = "?"; headerAvatar.style.background = "#7c4dff"; }
            if (headerName) headerName.textContent = "Sign In";
            if (menuFullName) menuFullName.textContent = "Guest";
            if (menuEmail) menuEmail.textContent = "Not logged in";
        }
    },
...
// ─────────────────────────────────────────────────────────────
// Init — chooses Firebase or localStorage auth automatically
// ─────────────────────────────────────────────────────────────
function init() {
    setupAuthListeners();
    setupListeners();
    ...
    // Restore session from stored JWT, if any
    api.me().then(user => {
        if (user) {
...
// ─────────────────────────────────────────────────────────────
// Application Data Layer — synced with the backend API
// ─────────────────────────────────────────────────────────────
...
async function loadState() {
    ...
    state.filters = new Set(['lecture', 'lab', 'study', 'assignment', 'default']);

    try {
        const d = await api.getState();
        if (d.events) state.events = d.events;
        if (d.tasks) state.tasks = d.tasks;
        if (d.focusTime !== undefined) state.focusTime = d.focusTime;
        if (d.filters && d.filters.length) state.filters = new Set(d.filters);
        if (d.categories && d.categories.length) state.categories = d.categories;
        if (d.dailyGoals) state.dailyGoals = d.dailyGoals;
    } catch (e) {
        console.error('loadState error:', e);
        showToast('Could not load your data from the server.', 'error');
    }
}

async function saveCategories() {
    if (!state.currentUser) return;
    await saveMainDoc({ categories: state.categories });
}
...
// ── Persistence: syncs the whole state blob to the backend, debounced ──
let _saveTimer = null;
...
// Main save — debounced so rapid edits (dragging, typing) don't spam the API
function saveState() {
    if (!state.currentUser) return;
    clearTimeout(_saveTimer);
    _saveTimer = setTimeout(_pushStateToServer, 400);
}
...
// Event / task CRUD — the whole state blob is persisted together, so these
// all just trigger a debounced save.
async function fsAddEvent(evt) { saveState(); }
async function fsUpdateEvent(evt) { saveState(); }
async function fsDeleteEvent(evt) { saveState(); }
async function fsAddTask(task) { saveState(); }
async function fsUpdateTask(task) { saveState(); }
async function fsDeleteTask(task) { saveState(); }
...
function setupAuthListeners() {
    ...
    // Registration avatar color selector
    document.querySelectorAll(".avatar-color-picker .avatar-opt").forEach(opt => {
        opt.addEventListener("click", () => {
            ...
            if (profileDisplay) {
                profileDisplay.style.backgroundColor = opt.getAttribute("data-color");
            }
        });
    });
    ...
    if (regPasswordInput && strengthBar) {
        regPasswordInput.addEventListener("input", (e) => {
            ...
            if (val.length >= 10 || /[A-Z]/.test(val)) score += 33;
    ...
    // Sign In form submit — Hybrid Auth
    if (loginForm) {
        loginForm.addEventListener('submit', async (e) => {
            ...
            if (btn) { btn.disabled = false; btn.innerHTML = '<span>Sign In to Dashboard</span>'; }
        });
    }

    // Google Sign-In isn't wired up for the custom backend — hide it
    ...
    // Create Account form submit — Hybrid Auth
    if (registerForm) {
    ...
    // Header User Profile Menu Dropdown Toggle
    ...
    if (userMenuBtn && userDropdownCard) {
        userMenuBtn.addEventListener("click", (e) => {
            ...
            userDropdownCard.classList.toggle("hidden");

            // Update stats inside dropdown menu
            const dropTasks = document.getElementById("drop-stat-tasks");
            const dropFocus = document.getElementById("drop-stat-focus");
            if (dropTasks) dropTasks.textContent = state.tasks.filter(t => t.completed).length;
            if (dropFocus) dropFocus.textContent = Math.round(state.focusTime) + "m";
        });

        document.addEventListener("click", (e) => {
            if (!userDropdownCard.contains(e.target) && !userMenuBtn.contains(e.target)) {
                userDropdownCard.classList.add("hidden");
    ...
    if (themeToggleBtn) {
        themeToggleBtn.addEventListener("click", () => AuthManager.toggleTheme());
    }
    ...
    if (openProfileBtn && profileModal) {
        openProfileBtn.addEventListener("click", () => {
            ...
            document.getElementById("profile-name-heading").textContent = user.fullName || user.username;
            ...
            document.getElementById("profile-avatar-color").value = user.avatarColor || "#40c4ff";

            const profileAvatar = document.getElementById("profile-avatar-display");
            if (profileAvatar) {
                profileAvatar.textContent = (user.fullName || user.username || "U").charAt(0).toUpperCase();
                profileAvatar.style.backgroundColor = user.avatarColor || "#40c4ff";
            }
            ...
            if (picker) {
                picker.querySelectorAll(".avatar-opt").forEach(opt => {
                    if (opt.getAttribute("data-color") === (user.avatarColor || "#40c4ff")) {
                        opt.classList.add("selected");
                    } else {
        ...
        const closeProf = () => profileModal.classList.add("hidden");
        if (closeProfileBtn) closeProfileBtn.addEventListener("click", closeProf);
        if (cancelProfileBtn) cancelProfileBtn.addEventListener("click", closeProf);
        profileModal.addEventListener("click", (e) => {
            if (e.target === profileModal) closeProf();
        });
    }
    ...
    if (profileForm) {
        profileForm.addEventListener("submit", async (e) => {
            ...
            const newColor = document.getElementById("profile-avatar-color").value || "#40c4ff";
            ...
            try {
                const patch = { fullName: user.fullName, avatarColor: user.avatarColor };
                if (newPass && newPass.length >= 6) patch.password = newPass;
                const updated = await api.updateProfile(patch);
                state.currentUser = { ...state.currentUser, ...updated };
            } catch (err) {
                showToast(err.message || 'Could not update account settings.', 'error');
                return;
            }
...
function setupListeners() {
    ...
    dom.addEventForm.addEventListener("submit", async (e) => { await saveEvent(e); });
...
async function saveEvent(e) {
    ...
    if (eventId) {
        ...
        if (evtIndex !== -1) {
            ...
            await fsUpdateEvent(state.events[evtIndex]);
...
async function deleteEvent(id) {
    ...
    if (evt) await fsDeleteEvent(evt);
